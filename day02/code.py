...
class Game:
    ...
    def setup(self, input_line):
        self.id = int(re.search(r"Game (\d+)", input_line).group(1))

        for set in re.findall(r"[^;]+", input_line):
            reds = re.search(r"(\d+)\s+red", set)
            n_reds = int(reds.group(1)) if reds else 0
            self.red_cubes.append(n_reds)

            greens = re.search(r"(\d+)\s+green", set)
            n_greens = int(greens.group(1)) if greens else 0
            self.green_cubes.append(n_greens)

            blues = re.search(r"(\d+)\s+blue", set)
            n_blues = int(blues.group(1)) if blues else 0
            self.blue_cubes.append(n_blues)

    def validate(self):
        MAX_N_RED_CUBES = 12
        MAX_N_GREEN_CUBES = 13
        MAX_N_BLUE_CUBES = 14
        return (
            not any(red > MAX_N_RED_CUBES for red in self.red_cubes)
            and not any(green > MAX_N_GREEN_CUBES for green in self.green_cubes)
            and not any(blue > MAX_N_BLUE_CUBES for blue in self.blue_cubes)
        )

    def compute(self):
        return max(self.red_cubes) * max(self.green_cubes) * max(self.blue_cubes)
