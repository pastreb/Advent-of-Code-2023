...
def compute_part_1(input_file_name="input.txt"):
    ...
    times = [int(time) for time in re.findall(r"\d+", input[0])]
    record_distances = [int(distance) for distance in re.findall(r"\d+", input[1])]
    n_races = len(times)
    numbers_of_ways = [0 for x in range(n_races)]
    product_of_numbers_of_ways = 1
    for race in range(n_races):
        time = times[race]
        for k in range(time):
            numbers_of_ways[race] += k * (time - k) > record_distances[race]
        product_of_numbers_of_ways *= numbers_of_ways[race]
    return product_of_numbers_of_ways
...
def compute_part_2(input_file_name="input.txt"):
    input = read_input(input_file_name)
    time = [int(time) for time in re.findall(r"\d+", input[0].replace(" ", ""))][0]
    record_distance = [
        int(distance) for distance in re.findall(r"\d+", input[1].replace(" ", ""))
    ][0]

    # Binary Search for Lower Bound:
    low_1 = 0
    high_1 = time // 2  # definitely breaks record
    while low_1 < high_1 - 1:
        mid = (low_1 + high_1) // 2
        if mid * (time - mid) < record_distance:
            low_1 = mid
        else:
            high_1 = mid

    # Binary Search for Upper Bound:
    low_2 = time // 2  # definitely breaks record
    high_2 = time
    while low_2 < high_2 - 1:
        mid = (low_2 + high_2) // 2
        if mid * (time - mid) >= record_distance:
            low_2 = mid
        else:
            high_2 = mid
    return low_2 - high_1 + 1
