...
def compute_part_1():
    ...
    for line in input:
        ...
        sum += line_numbers[0]*10 + line_numbers[len(line_numbers)-1]
    return sum
...
def replace_spelled_digits(s):
    ...
    while(i < len(s)):
        found_one = False
        for digit in DIGITS:
            if(s[i:i+len(digit[0])] == digit[0]):
                new_s += digit[1]
                found_one = True
                break
        if not found_one:
            new_s += s[i]
        i += 1 # The right calibration values for string "eighthree" is 83 and for "sevenine" is 79.
    return new_s
