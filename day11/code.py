...
def expand(universe):
    ...
    while i < len(universe):
        ...
        i += 1
        if not "#" in row:
            universe.insert(i, row.copy())
            i += 1
    # Expand Columns
    j = 0
    while j < len(universe[0]):
        column = [universe[i][j] for i in range(len(universe))]
        j += 1
        if not "#" in column:
...
def compute_part_1(input_file_name="input.txt"):
    ...
    universe = [[x for x in re.findall(r"\.|\#", line)] for line in input]
    expand(universe)
    galaxies = get_galaxy_coordinates(universe)
    sum_of_distances = 0
    for i in range(len(galaxies)):
        for j in range(i + 1, len(galaxies)):
            sum_of_distances += abs(galaxies[i][0] - galaxies[j][0]) + abs(
...
def get_sum_of_distances(universe, factor):
    ...
    for i in range(len(galaxies)):
        for j in range(i + 1, len(galaxies)):
            x0 = min(galaxies[i][0], galaxies[j][0])
            x1 = max(galaxies[i][0], galaxies[j][0])
            y0 = min(galaxies[i][1], galaxies[j][1])
            y1 = max(galaxies[i][1], galaxies[j][1])
            sum_of_distances += (
                (x1 - x0)
                + (y1 - y0)
                + (factor - 1)
                * (
                    sum(1 for i in empty_rows if i > x0 and i < x1)
                    + sum(1 for j in empty_columns if j > y0 and j < y1)
                )
            )  # Simple Manhattan Distance with Expansion
    return sum_of_distances
...
def compute_part_2(input_file_name="input.txt"):
    input = read_input(input_file_name)
    universe = [[x for x in re.findall(r"\.|\#", line)] for line in input]
    return get_sum_of_distances(universe, 1000000)
