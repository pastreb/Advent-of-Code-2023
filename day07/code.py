...
def check_five(hand):
    ...
    return hand_sorted[0] == hand_sorted[4]
...
def check_four(hand):
    if len(hand) != 5:
        return False
    hand_sorted = sorted(hand)
    return (hand_sorted[0] == hand_sorted[3]) or (hand_sorted[1] == hand_sorted[4])
...
def check_full_house(hand):
    if len(hand) != 5:
        return False
    hand_sorted = sorted(hand)
    return (hand_sorted[0] == hand_sorted[1] and hand_sorted[2] == hand_sorted[4]) or (
        hand_sorted[0] == hand_sorted[2] and hand_sorted[3] == hand_sorted[4]
    )
...
def check_three(hand):
    ...
    hand_sorted = sorted(hand)
    return (
        (hand_sorted[0] == hand_sorted[2])
        or (hand_sorted[1] == hand_sorted[3])
        or (hand_sorted[2] == hand_sorted[4])
    )
...
def check_two_pairs(hand):
    ...
    hand_sorted = sorted(hand)
    return (
        (hand_sorted[0] == hand_sorted[1] and hand_sorted[2] == hand_sorted[3])
        or (hand_sorted[0] == hand_sorted[1] and hand_sorted[3] == hand_sorted[4])
        or (hand_sorted[1] == hand_sorted[2] and hand_sorted[3] == hand_sorted[4])
    )
...
def check_pair(hand):
    ...
    hand_sorted = sorted(hand)
    return (
        (hand_sorted[0] == hand_sorted[1])
        or (hand_sorted[1] == hand_sorted[2])
        or (hand_sorted[2] == hand_sorted[3])
        or (hand_sorted[3] == hand_sorted[4])
    )
...
def compute_part_1(input_file_name="input.txt"):
    ...
    for line in input:
        hand, bid = line.split(" ")
        hands_n_bids.append([hand, int(bid)])
    sorted_hands_n_bids = sorted(hands_n_bids, key=lambda x: evaluate_hand(x[0]))
    winnings = 0
    for i in range(len(sorted_hands_n_bids)):
        winnings += sorted_hands_n_bids[i][1] * (i + 1)
    return winnings
...
def evaluate_hand_in_joker_mode(hand):
    max_value = 0
    for option in ["2", "3", "4", "5", "6", "7", "8", "9", "T", "Q", "K", "A"]:
        value = 0
        other_hand = hand.replace("J", option)
        if check_five(other_hand):
            value += 9
        elif check_four(other_hand):
            value += 8
        elif check_full_house(other_hand):
            value += 7
        elif check_three(other_hand):
            value += 6
        elif check_two_pairs(other_hand):
            value += 5
        elif check_pair(other_hand):
            value += 4
        else:
            value += 1
    ...
    for card in hand:
        max_value *= 100
        max_value += cards_to_values[card] if card != "J" else 0
    return max_value
...
def compute_part_2(input_file_name="input.txt"):
    ...
    for line in input:
        hand, bid = line.split(" ")
        hands_n_bids.append([hand, int(bid)])
    sorted_hands_n_bids = sorted(
        hands_n_bids, key=lambda x: evaluate_hand_in_joker_mode(x[0])
    )
    winnings = 0
    for i in range(len(sorted_hands_n_bids)):
        winnings += sorted_hands_n_bids[i][1] * (i + 1)
