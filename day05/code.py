...
# more sand soon; we only turned off the water a few days... weeks... oh no."
...
class Seed:
    ...
    def apply_map(self, map):  # same for any map
        for row in map:
            dest_start, source_start, range = row[0], row[1], row[2]
            if self.value >= source_start and self.value < source_start + range:
                self.value = dest_start + (self.value - source_start)
                return
        # If we never returned at this point, the value is mapped 1:1.

    def reverse_apply_map(self, map):  # same for any map
        for row in map:
            dest_start, source_start, range = row[0], row[1], row[2]
            if self.value >= dest_start and self.value < dest_start + range:
                self.value = source_start + (self.value - dest_start)
                return
        # If we never returned at this point, the value is mapped 1:1.


def collect_map_values(input, i):
    map = []
    while i < len(input):
        map_values = [int(map_value) for map_value in re.findall(r"\d+", input[i])]
        if (len(map_values)) == 0:
            return map
...
def compute_part_1(input_file_name="input.txt"):
    ...
    seeds = [Seed(int(seed_value)) for seed_value in re.findall(r"\d+", input[0])]
    for i in range(1, len(input)):
        if "map" in input[i]:  # new map
            map = collect_map_values(input, i + 1)
            for seed in seeds:
                seed.apply_map(map)
    return min([seed.value for seed in seeds])
...
def check_if_initial_seed(seed_value, seed_values):
    for i in range(len(seed_values) // 2):
        if (
            seed_value >= seed_values[2 * i]
            and seed_value < seed_values[2 * i] + seed_values[2 * i + 1]
        ):
            return True
    return False


def compute_part_2(input_file_name="input.txt"):  # This is like reeeeeaaally slow
    input = read_input(input_file_name)
    seed_values = [int(seed_value) for seed_value in re.findall(r"\d+", input[0])]
    maps = []
    for i in range(1, len(input)):
        if "map" in input[i]:  # new map
            map = collect_map_values(input, i + 1)
    ...
    while True:
        seed = Seed(i)
        for map in maps:
            seed.reverse_apply_map(map)
        if check_if_initial_seed(seed.value, seed_values):
            return i
        print(i)
        i += 1
