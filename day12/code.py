...
def validate_row(springs, numbers):
    springs = ''.join(springs)
    broken_springs = re.findall(r"\#+", springs)
    # print(springs, broken_springs)
    if len(broken_springs) != len(numbers):
        return False
    for i in range(len(broken_springs)):
        if len(broken_springs[i]) != numbers[i]:
            return False
    return True

def compute_arrangements_in_row(springs, numbers, index):
    if index == len(springs):
        return 1 if validate_row(springs, numbers.copy()) else 0
    if springs[index] == "?":
        sum = 0
        springs[index] = "."
        sum += compute_arrangements_in_row(springs, numbers, index+1)
        springs[index] = "#"
        sum += compute_arrangements_in_row(springs, numbers, index+1)
        springs[index] = "?"
        return sum
    return compute_arrangements_in_row(springs, numbers, index+1)


def compute_part_1(input_file_name="input.txt"):
    input = read_input(input_file_name)
    springs = [[spring for spring in row if spring in [".", "#", "?"]] for row in input]
    numbers = [[int(number) for number in re.findall(r"\d+", row)] for row in input]
    total_n_arrangements = 0
    for i in range(len(springs)):
        total_n_arrangements += compute_arrangements_in_row(springs[i], numbers[i], 0)
        print(total_n_arrangements)
...
def get_unfolded(input):
    ...
    for row in springs:
        ...
        for _ in range(5):
            for spring in row:
                unfolded_row.append(spring)
            unfolded_row.append("?")
        unfolded_springs.append(unfolded_row)
    numbers = [[int(number) for number in re.findall(r"\d+", row)] for row in input]
    unfolded_numbers = []
    for row in numbers:
        unfolded_row = []
        for _ in range(5):
            for number in row:
                unfolded_row.append(number)
        unfolded_numbers.append(unfolded_row)
    return unfolded_springs, unfolded_numbers

def dp(springs, numbers):
    n = len(springs)
    m = springs.count("?")
    dp_working = [[0 for _ in range(n)] for _ in range(m)]
    dp_broken = [[0 for _ in range(n)] for _ in range(m)]
    # dp_working[i][j] contains the number of possible arrangements 
    # for springs[i:n] that satisfy j numbers 
    # if springs[i] was "?" and becomes "."
...
def compute_part_2(input_file_name="input.txt"):
    input = read_input(input_file_name)
    springs, numbers = get_unfolded(input)
    print(numbers)
...
if __name__ == "__main__":
    ...
    print(f"PART 2: {compute_part_2('sample_input.txt')}")
