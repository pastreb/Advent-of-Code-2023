...
def count_matches(line):
    numbers = [[int(x) for x in re.findall(r"\d+", part)] for part in line.split("|")]
    winning_numbers = numbers[0][1:]  # remove game number
    my_numbers = numbers[1]
    matches = len([x for x in my_numbers if x in winning_numbers])
    return matches


def compute_part_1(input_file_name="input.txt"):
    input = read_input(input_file_name)
    return sum([int(2 ** (count_matches(line) - 1)) for line in input])
...
def compute_part_2(input_file_name="input.txt"):
    input = [[line, 1] for line in read_input(input_file_name)]  # cards with counts
    for i in range(len(input)):
        line = input[i][0]
        count = input[i][1]
        n_matches = count_matches(line)
        for j in range(i + 1, min(i + 1 + n_matches, len(input))):
            input[j][1] += count
    return sum([line[1] for line in input])
