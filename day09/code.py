...
def get_sequence_of_histories(line):
    sequence_of_histories = [[int(x) for x in re.findall(r"\-?\d+", line)]]
    while not check_all_zeros(sequence_of_histories[-1]):
        history = sequence_of_histories[-1]
        new_history = []
        for i in range(len(history) - 1):
            new_history.append(history[i + 1] - history[i])
        sequence_of_histories.append(new_history)
    return sequence_of_histories
...
def compute_part_1(input_file_name="input.txt"):
    ...
    for line in input:
        # Differences
        sequence_of_histories = get_sequence_of_histories(line)
        # Extrapolation
        sum_of_next_values += sum([history[-1] for history in sequence_of_histories])
...
def compute_part_2(input_file_name="input.txt"):
    ...
    for line in input:
        ...
        sequence_of_histories = get_sequence_of_histories(line)
        # Extrapolation
        for i in range(len(sequence_of_histories) - 2, -1, -1):
            sequence_of_histories[i].insert(
                0, sequence_of_histories[i][0] - sequence_of_histories[i + 1][0]
            )
        sum_of_prev_values += sequence_of_histories[0][0]
